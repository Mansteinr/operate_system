    ...
    <Inquiry :queryParams="queryParams" @initFun="initFun">
      <div class="show-query query-form">
        <el-form-item label="选择日期：" prop="date">
          <div class="block">
            <el-date-picker v-model="queryParams.date" type="date" name="date" placeholder="选择日期"></el-date-picker>
          </div>
        </el-form-item>
        <el-form-item label="开始时间：" prop="start1">
          <div class="block">
            <el-time-picker v-model="queryParams.start1" name="start" placeholder="选择开始时间"></el-time-picker>
          </div>
        </el-form-item>
        <el-form-item label="结束时间：" prop="end1">
          <div class="block">
            <el-time-picker v-model="queryParams.end1" name="end" placeholder="选择结束时间"></el-time-picker>
          </div>
        </el-form-item>
        <Select
      ...
      <div class="qurey-btn" style="margin: 10px 0 0 10px;">
        ...
        >{{showHideFlag?"显示":"隐藏"}}</el-button>
      </div>
      <div class="query-hide" ref="paramsBox">
        <el-form-item v-for="(v, key) in queryParamsByServiceNameHandleList" :label="`${v.paramNameCh}：`" :key="key">
          <el-input :placeholder="`请选${v.paramNameCh}`" :name="v.paramName"></el-input>
        </el-form-item>
      </div>
    </Inquiry>
  ...
  methods: {
    ...
    formatterSrc(val) {
      ...
      if (val && val.length) {
        ...
        val.forEach(v => {
          if (v.className) {
            html += `<span class="param-item" title="渠道名称: ${
              v.className.split(".")[2]
            }  ${v["invokeCostTime"]}">渠道名称: ${
              v.className.split(".")[2]
            }  ${v["invokeCostTime"]}</span>`;
          }
        });
        return html;
      }
    ...
    showHideToggle() {
      ...
      if (this.showHideFlag) {
        this.queryParamsByServiceNameAjax(op)
      }
    },
    formatter(val) {
    ...
    ...mapActions({
      ...
      queryParamsByServiceNameAjax: "tools/queryParamsByServiceNameAjax"
    })
  },
  computed: {
    ...mapGetters({
    ...
    queryParamsByServiceNameHandleList() {
      return [...[{
              ...
              paramNameCh: "耗时小于"
            }], ...this.queryParamsByServiceNameList]
    }
  }
};
</script>
